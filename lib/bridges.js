...
function AmqpToHttpBridge(address, host, port, ioFogOptions = {}) {
    log.info('Created AMQP to HTTP bridge %s => %s:%s', address, host, port);
    this.host = host;
    this.port = port;
    this.address = address;
    this.routerHost = ioFogOptions.routerHost || 'localhost';
    this.routerPort = ioFogOptions.routerPort || 5672;
    var container = rhea.create_container({enable_sasl_external: !!ioFogOptions.saslEnabled});
    container.id = process.env.HOSTNAME + '_amqp_' + this.address + '_to_http_' + this.host  + '_' + this.port;

    const transport = process.env.TRANSPORT || null;
    const servername = process.env.SERVERNAME || null;
    const keyPath = process.env.TLS_KEY ? path.resolve(__dirname, process.env.TLS_KEY) : null;
    const certPath = process.env.TLS_CERT ? path.resolve(__dirname, process.env.TLS_CERT) : null;
    const caPath = process.env.TLS_CA ? path.resolve(__dirname, process.env.TLS_CA) : null;
    ...
    if (servername) {
        log.info('Setting servername to:', servername);
        connectionOptions.servername = servername;
    } else {
        log.info('TLS // servername option not set');
    }

    if (transport) {
        log.info('Setting transport to:', transport);
    ...
    if (keyPath && certPath && fs.existsSync(keyPath) && fs.existsSync(certPath)) {
        log.info('Setting TLS options (client certificate authentication)');
        connectionOptions.key = fs.readFileSync(keyPath);
        connectionOptions.cert = fs.readFileSync(certPath);
    } else {
        log.info('TLS // client certificate authentication options not set');
    }

    if (caPath && fs.existsSync(caPath)) {
        log.info('Setting TLS options (if the server uses the self-signed certificate)');
        connectionOptions.ca = [ fs.readFileSync(caPath) ];
    } else {
        log.info('TLS // client CA certificate authentication options not set');
    }

    log.info('Connecting with options:', connectionOptions);
...
AmqpToHttpBridge.prototype.incoming = function (context) {
	...
	response.on('end', function () {
            ...
            metrics.http_egress_request(self.metrics, self.host, options.method, response.statusCode, site, context.message.body ? context.message.body.length : 0, message_out.body ? message_out.body.length : 0, elapsed(start));
        });
    });
...
function HttpToAmqpBridge(port, address, ioFogOptions = {}) {
    ...
    this.client = rclient.create(container.connect(connectionOptions), address);
...
HttpToAmqpBridge.prototype.stop = function () {
    if (this.client) {
        this.client.close();
    }
...
function AmqpToTcpBridge(address, host, port, ioFogOptions = {}) {
    ...
    this.container = rhea.create_container({enable_sasl_external: !!ioFogOptions.saslEnabled});
    this.container.id = process.env.HOSTNAME + '_amqp_' + address + '_to_tcp_' + this.host  + '_' + this.port;

    this.create_connection();
}

AmqpToTcpBridge.prototype.reconnect = function () {
    try {
        this.stop();
        this.create_connection();
    } catch (err) {
...
AmqpToTcpBridge.prototype.on_connection_error = function (error) {
    this.reconnect();
}

AmqpToTcpBridge.prototype.create_connection = function () {
    ...
    this.connection.on('connection_error', this.on_connection_error.bind(this));
    this.connection.on('error', this.on_connection_error.bind(this));
...
function TcpToAmqpBridge(port, address, ioFogOptions = {}) {
    ...
    this.routerHost = ioFogOptions.routerHost || 'localhost';
    this.routerPort = ioFogOptions.routerPort || 5672;
    this.container = rhea.create_container({enable_sasl_external: !!ioFogOptions.saslEnabled, reconnect:false});
...
TcpToAmqpBridge.prototype.incoming_connection = function (socket) {
    ...
    var connection = this.container.connect(connectionOptions);
    var self = this;
...
TcpToAmqpBridge.prototype.stop = function () {
    if (this.connection && this.connection.is_open()) {
        this.connection.close();
    }
    if (this.server && this.server.listening) {
...
module.exports.amqp_to_http = function (address, host, port, ioFogOptions) {
    return new AmqpToHttpBridge(address, host, port, ioFogOptions);
};
...
module.exports.amqp_to_tcp = function (address, host, port, ioFogOptions) {
    return new AmqpToTcpBridge(address, host, port, ioFogOptions);
};

module.exports.http_to_amqp = function (port, address, ioFogOptions) {
    return new HttpToAmqpBridge(port, address, ioFogOptions);
};
...
module.exports.tcp_to_amqp = function (port, address, ioFogOptions) {
    return new TcpToAmqpBridge(port, address, ioFogOptions);
};
