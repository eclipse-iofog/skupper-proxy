...
function AmqpToHttpBridge(address, host, port, routerHost, routerPort, saslEnabled) {
    log.info('Created AMQP to HTTP bridge %s => %s:%s', address, host, port);
    this.host = host;
    this.port = port;
    this.address = address;
    this.routerHost = routerHost || 'localhost';
    this.routerPort = routerPort || 5672;
    var container = rhea.create_container({enable_sasl_external: !!saslEnabled});
...
function HttpToAmqpBridge(port, address, routerHost, routerPort, saslEnabled) {
...
module.exports.amqp_to_http = function (address, host, port, routerHost, routerPort, saslEnabled) {
    return new AmqpToHttpBridge(address, host, port, routerHost, routerPort, saslEnabled);
};
...
module.exports.http_to_amqp = function (port, address, routerHost, routerPort, saslEnabled) {
    return new HttpToAmqpBridge(port, address, routerHost, routerPort, saslEnabled);
};
