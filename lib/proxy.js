...
function Proxy(config, bridgehost, routerHost, routerPort, saslEnabled) {
  ...
  if (bridgeconfig) {
    switch (bridgeconfig.type) {
      case "amqp_to_http":
        return bridges.amqp_to_http(bridgeconfig.source, bridgehost, bridgeconfig.target, routerHost, routerPort, saslEnabled)
      case "amqp_to_http2":
        return bridges.amqp_to_http2(bridgeconfig.source, bridgehost, bridgeconfig.target)
      case "amqp_to_tcp":
        return bridges.amqp_to_tcp(bridgeconfig.source, bridgehost, bridgeconfig.target)
      case "http_to_amqp":
        return bridges.http_to_amqp(bridgeconfig.source, bridgeconfig.target, routerHost, routerPort, saslEnabled);
      case "http2_to_amqp":
        return bridges.http2_to_amqp(bridgeconfig.source, bridgeconfig.target);
      case "tcp_to_amqp":
        return bridges.tcp_to_amqp(bridgeconfig.source, bridgeconfig.target);
    }
  }
...
module.exports.proxy = function (config, bridgehost, routerHost, routerPort, saslEnabled) {
  return new Proxy(config, bridgehost, routerHost, routerPort, saslEnabled)
}